...
lineaccesstoken = 'v0lYjr/s+drvGscTRr2vPDKD2Pj17UGZ9j7j9ApADKlFERFcDtjR0VNSdKJKMu2HC3BPn6k9+prw3IgElnoiZU6eEmn1lKCYvygsQChE1xV8B1la/7OtoLCswaxkJzvEHESUE7TdNNRhsSjgTR2nzwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="ดีจรา")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "อยากไปเที่ยวปะ") :
            replyObj = TextSendMessage(text="ค่ดอยากแม่")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวมั้ย") :
            replyObj = TextSendMessage(text="ก้ไปเลยสิคะ")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/52124ad7-9d74-4b01-bbb7-2cfa15f2b116"
